...
var yamlToReleases = function yamlToReleases(changesets) {
  return changesets.map(function (changeset) {
    ...
    var yamlStuff = yaml.safeLoad(roughReleases);
...
var legacy = legacyChangesetRepos.indexOf(repoid) >= 0;
var v2 = v2ChangesetRepos.indexOf(repoid) >= 0;
var changesetPromise = legacy ? getCommits(user, repo, pullrequestid) : getChangesets(user, repo, sourcehash, destinationhash, v2);
changesetPromise.then(function (changesets) {
  ...
  var releases = v2 ? yamlToReleases(changesets) : flattenChangesets(changesets);
