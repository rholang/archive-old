    ...
    MockReactionsClient.prototype.getDetailedReaction = function (containerAri, ari, emojiId) {
        ...
        return this.delayPromise().then(function () {
            ...
            if (reactionsMockData) {
                ...
                if (reaction_1) {
                    var users = __spread(defaultUsers).slice(Math.floor(Math.random() * 4), Math.floor(Math.random() * 9) + 4)
                        .slice(0, reaction_1.count);
                    return __assign(__assign({}, reaction_1), { users: users });
                }
            }
            return {
                containerAri: containerAri,
            ...
            if (reactionsMockData) {
                _this.mockData[reactionKey] = reactionsMockData.map(function (reaction) {
                    if (reaction.emojiId === emojiId) {
                        found = true;
                        return __assign(__assign({}, reaction), { count: reaction.count + 1, reacted: true });
                    }
                    return reaction;
                });
            }
            if (!found) {
                _this.mockData[reactionKey] = __spread((reactionsMockData ? reactionsMockData : []), [
                    {
                        containerAri: containerAri,
                        ari: ari,
                        emojiId: emojiId,
        ...
        return this.delayPromise().then(function () {
            ...
            _this.mockData[reactionKey] = _this.mockData[reactionKey]
                ...
                if (reaction.emojiId === emojiId) {
                    ...
                    return __assign(__assign({}, reaction), { count: reaction.count - 1, reacted: false });
                }
                return reaction;
            })
