...
var matchAllowedDomains = memoize_one_1.default(function (domain, config) {
    return (config &&
 ...
 */
exports.showInviteWarning = function (config, selectedUsers) {
    if (config && selectedUsers) {
        var mode = config.mode;
        var selectedEmails = Array.isArray(selectedUsers)
            ? selectedUsers.filter(user_picker_1.isEmail)
            : [selectedUsers].filter(user_picker_1.isEmail);
...
exports.optionDataToUsers = function (optionDataArray) {
...
exports.allowEmails = function (config) {
...
exports.isValidEmailUsingConfig = memoize_one_1.default(function (config) {
    var checkDomain = needToCheckDomain(config);
    return function (inputText) {
        var result = user_picker_1.isValidEmail(inputText);
        if (result === 'VALID' &&
            checkDomain &&
