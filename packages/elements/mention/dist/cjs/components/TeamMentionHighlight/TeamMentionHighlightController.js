...
exports.mentionHighlightLocalStorageKey = 'atlassian.people.context.team.mention.highlight';
var EMPTY = {
    seenCount: 0,
    dontShow: false,
};
...
var TeamMentionHighlightController = /** @class */ (function () {
    ...
    TeamMentionHighlightController.readFromLocalStorage = function () {
        try {
            var localVal = localStorage.getItem(exports.mentionHighlightLocalStorageKey);
            if (!localVal) {
                // Attempt to write to see if the user has local storage access
                localStorage.setItem(exports.mentionHighlightLocalStorageKey, JSON.stringify(EMPTY));
                localVal = localStorage.getItem(exports.mentionHighlightLocalStorageKey);
            }
            if (localVal) {
    ...
    TeamMentionHighlightController.saveToLocalStorage = function (item) {
        try {
            localStorage.setItem(exports.mentionHighlightLocalStorageKey, JSON.stringify(item));
        }
        catch (err) {
            // do nothing
        }
