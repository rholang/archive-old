...
var Article = /** @class */ (function (_super) {
    ...
    Article.prototype.renderArticleContent = function () {
        ...
        if (currentArticle) {
            ...
            if (currentArticle.state === REQUEST_STATE.error) {
                return React.createElement(LoadingError, null);
            }
            else if (article && currentArticle.state === REQUEST_STATE.done) {
                return (React.createElement(React.Fragment, null,
                    React.createElement(ArticleContent, { title: article.title, body: article.body, titleLinkUrl: article.productUrl }),
                    React.createElement(ArticleWasHelpfulForm, null),
                    React.createElement(RelatedArticles, { relatedArticles: article.relatedArticles, onRelatedArticlesListItemClick: handleOnClick })));
            }
            else {
                return (React.createElement(React.Fragment, null,
                    React.createElement(ArticleContent, { isLoading: true }),
                    React.createElement(RelatedArticles, { isLoading: true })));
            }
        }
        return null;
    };
    Article.prototype.render = function () {
        var _this = this;
        var skipArticleFadeInAnimation = this.state.skipArticleFadeInAnimation;
        return (React.createElement(Transition, { in: this.props.help.isArticleVisible(), timeout: TRANSITION_DURATION_MS, enter: !skipArticleFadeInAnimation, onEntered: this.onArticleEntered, onExited: this.onArticleExited, mountOnEnter: true, unmountOnExit: true }, function (state) { return (React.createElement(ArticleContainer, { ref: _this.refArticleContainer, isSearchVisible: _this.props.help.isSearchVisible(), style: __assign(__assign({}, defaultStyle), transitionStyles[state]) }, _this.renderArticleContent())); }));
    };
    return Article;
}(Component));
export { Article };
