...
var filterText = function (acc, node) {
    ...
    return lines.reduce(function (acc, line, index) {
        ...
        acc.push(tslib_1.__assign(tslib_1.__assign({}, node), { text: line }));
        return acc;
    }, acc);
};
var filterContent = function (content, types, schema, breakBetweenBlocks) {
    return content.reduce(function (acc, node) {
        if (types.has(node.type)) {
            if (node.content) {
                acc.push(tslib_1.__assign(tslib_1.__assign({}, node), { content: filterContent(node.content, types) }));
            }
...
exports.filterSliceByType = function (slice, types, schema, breakBetweenBlocks) {
    ...
    return prosemirror_model_1.Slice.fromJSON(schema, {
