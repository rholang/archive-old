...
exports.updateColumnWidths = function (resizeState, table, start) { return function (tr) {
    ...
    if (selection instanceof prosemirror_state_1.TextSelection) {
        tr.setSelection(new prosemirror_state_1.TextSelection(tr.doc.resolve(selection.$from.pos), tr.doc.resolve(selection.$to.pos)));
    }
    else if (selection instanceof prosemirror_tables_1.CellSelection) {
        var newSelection = prosemirror_tables_1.CellSelection.create(tr.doc, selection.$anchorCell.pos, selection.$headCell.pos);
