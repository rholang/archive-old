...
exports.deleteColumns = function (rect) { return function (tr) {
            ...
            if (columnsToDelete.indexOf(colIndex) === -1) {
                ...
                if (cellsInColumn.indexOf(cellPos) > -1 && !seen[cellPos]) {
                    ...
                    if (overlappingCols_1 > 0) {
                        var attrs = tslib_1.__assign(tslib_1.__assign({}, cell.attrs), { colspan: cell.attrs.colspan - overlappingCols_1 });
                    ...
                    var attrs = tslib_1.__assign(tslib_1.__assign({}, cell.attrs), { colspan: 1, rowspan: 1 });
                    if (cell.attrs.colwidth) {
    ...
    if (!rows.length) {
        return metadata_1.setMeta({ type: 'DELETE_COLUMNS', problem: 'EMPTY_TABLE' })(tr);
    }
    var newTable = table.node.type.createChecked(table.node.attrs, rows, table.node.marks);
    var fixedTable = fixRowSpans(newTable);
    if (fixedTable === null) {
        return metadata_1.setMeta({ type: 'DELETE_COLUMNS', problem: 'FIX_ROWSPANS' })(tr);
    }
    var cursorPos = getNextCursorPos(newTable, columnsToDelete);
    return metadata_1.setMeta({ type: 'DELETE_COLUMNS' })(tr
        ...
        .setSelection(prosemirror_state_1.Selection.near(tr.doc.resolve(table.pos + cursorPos))));
}; };
    ...
    for (var rowIndex = 0; rowIndex < map.height; rowIndex++) {
        ...
        if (minRowSpans[rowIndex] === 1) {
            ...
            for (var colIndex = 0; colIndex < row.childCount; colIndex++) {
                ...
                var newCell = cell.type.createChecked(tslib_1.__assign(tslib_1.__assign({}, cell.attrs), { rowspan: rowspan }), cell.content, cell.marks);
                rowCells.push(newCell);
            }
            rows.push(row.type.createChecked(row.attrs, rowCells, row.marks));
        }
