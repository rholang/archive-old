...
var tableSerializer_1 = tslib_1.__importDefault(require("./tableSerializer"));
/**
...
var generateOuterBacktickChain = (function () {
    ...
    return function (text, minLength) {
        ...
        return util_1.stringRepeat('`', length);
    };
})();
    ...
    MarkdownSerializerState.prototype.renderContent = function (parent) {
        ...
        parent.forEach(function (child, _offset, index) {
            ...
            child.isTextblock &&
                ...
                return exports.nodes.empty_line(_this, child);
            }
...
}(prosemirror_markdown_1.MarkdownSerializerState));
...
}(prosemirror_markdown_1.MarkdownSerializer));
...
var editorNodes = {
    ...
    image: function (state, node) {
        ...
        state.write('![' +
            util_1.escapeMarkdown(node.attrs.alt) +
            '](' +
            node.attrs.src +
            (node.attrs.title ? " '" + util_1.escapeMarkdown(node.attrs.title) + "'" : '') +
            ')');
    },
    ...
    text: function (state, node, parent, index) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            state.write();
            state.out += util_1.escapeMarkdown(lines[i], startOfLine, state.context.insideTable);
            if (i !== lines.length - 1) {
                if (lines[i] &&
                    lines[i].length &&
                    lines[i + 1] &&
...
exports.nodes = tslib_1.__assign(tslib_1.__assign({}, editorNodes), tableSerializer_1.default);
exports.marks = {
