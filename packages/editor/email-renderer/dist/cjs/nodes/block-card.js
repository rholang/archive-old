...
var className = util_1.createClassName('blockCard');
...
var renderBlockCardWithData = function (attrs) {
    ...
    var heading = create_tag_1.createTag('div', { class: className + '-headingData' }, name);
    var text = create_tag_1.createTag('div', { class: className + '-contentTextWithData' }, summary);
    var blockContent = table_util_1.createTable([
        [
            {
...
var renderBlockCard = function (attrs, text) {
    ...
    var heading = create_tag_1.createTag('div', { class: className + "-block " + className + "-headingUrl" }, title);
    return table_util_1.createTable([[{ attrs: { class: className + '-outerTd' }, text: heading }]], {}, { class: className + '-block' });
};
function blockCard(_a) {
    var attrs = _a.attrs, text = _a.text;
    if (attrs.data) {
        var href_1 = attrs.data.url;
        var card_1 = renderBlockCardWithData(attrs);
        return href_1
            ? create_tag_1.createTag('a', { href: href_1, class: className + '-link' }, card_1)
            : card_1;
    }
    var href = attrs.url;
    var card = renderBlockCard(attrs, text);
    return href
        ? create_tag_1.createTag('a', { href: href, class: className + '-link' }, card)
        : card;
}
