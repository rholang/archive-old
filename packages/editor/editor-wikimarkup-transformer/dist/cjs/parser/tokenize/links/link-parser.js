...
function extractLinkBody(buffer) {
    ...
    var body = new text_1.StringBuffer();
    var inEscape = false;
...
function extractNumber(buf) {
    var digits = new text_1.StringBuffer();
    var i = 0;
    for (; i < buf.length() && text_1.isDigit(buf.charAt(i)); i++) {
...
function parseLink(linkText) {
    ...
    var buf = new text_1.StringBuffer(linkText);
...
function parseContentLink(link) {
    ...
    if (!notLinkBody.startsWith('~')) {
        var shortcutBuf = new text_1.StringBuffer(notLinkBody);
        shortcutName = trimIfPossible(divideAfterLast(shortcutBuf, '@'));
        if (text_1.isNotBlank(shortcutName)) {
            shortcutValue = shortcutBuf.toString();
        }
    }
    var buf = new text_1.StringBuffer(notLinkBody);
    if (text_1.isBlank(shortcutName)) {
        spaceKey = trimIfPossible(divideOn(buf, ':'));
        if (buf.indexOf('$') === 0) {
            buf.deleteCharAt(0);
            contentId = extractNumber(buf);
            if (contentId === 0) {
                return tslib_1.__assign(tslib_1.__assign({}, link), { shortcutName: shortcutName,
