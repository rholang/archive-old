    ...
    while (index < input.length) {
        ...
        switch (state) {
            case processState.NEW_LINE: {
                ...
                if (listMatch) {
                    var _b = __read(listMatch, 2), symbols = _b[1];
                    ...
                    if (!builder) {
                        ...
                        if (buffer.length > 0) {
                            ...
                            contentBuffer.push.apply(contentBuffer, __spread(content));
                            builder.add([
                                {
                        ...
                        if (type !== builder.type && symbols.length === 1) {
                            output.push.apply(output, __spread(builder.buildPMNode()));
                            builder = new ListBuilder(schema, symbols);
                        }
                    }
            ...
            case processState.MACRO: {
                ...
                if (token.type === 'text') {
                    ...
                    if (buffer.length > 0) {
                        ...
                        contentBuffer.push.apply(contentBuffer, __spread(sanitize(content, schema)));
                        buffer = '';
                    }
                    contentBuffer.push.apply(contentBuffer, __spread(sanitize(token.nodes, schema)));
                }
                index += token.length;
                state = processState.BUFFER;
                continue;
            ...
            case 'heading': {
                ...
                curr.content.forEach(function (n) {
                    ...
                    if (n.type.name === 'text') {
                        ...
                        contentBuffer_1.push(n.mark(__spread(n.marks, [mark])));
                    }
                    else {
