...
function normalizeInlineNodes(nodes, schema) {
    ...
    try {
        for (var nodes_1 = __values(nodes), nodes_1_1 = nodes_1.next(); !nodes_1_1.done; nodes_1_1 = nodes_1.next()) {
            ...
            if (trimedInlineNodes_1.length > 0) {
                output.push.apply(output, __spread(createParagraphNodeFromInlineNodes(trimedInlineNodes_1, schema)));
            }
    ...
    if (trimedInlineNodes.length > 0) {
        output.push.apply(output, __spread(createParagraphNodeFromInlineNodes(trimedInlineNodes, schema)));
    }
    if (output.length === 0) {
