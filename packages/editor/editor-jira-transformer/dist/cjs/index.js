...
var fix_doc_1 = tslib_1.__importDefault(require("./fix-doc"));
...
var JIRATransformer = /** @class */ (function () {
    ...
    JIRATransformer.prototype.parse = function (html) {
        ...
        var dom = fix_doc_1.default(parse_html_1.default(html)).querySelector('body');
        var nodes = utils_1.bfsOrder(dom);
        ...
        for (var i = nodes.length - 1; i >= 0; i--) {
            ...
            var candidate = utils_1.convert(content_1, node, this.schema);
        ...
        var compatibleContent = this.schema.nodes.doc.validContent(content)
            ? content
            : utils_1.ensureBlocks(content, this.schema);
    ...
    JIRATransformer.prototype.encodeNode = function (node) {
        ...
        if (adf_schema_1.isSchemaWithLists(this.schema)) {
        ...
        if (adf_schema_1.isSchemaWithMentions(this.schema) && node.type === mention) {
            return this.encodeMention(node, this.customEncoders.mention);
        }
        if (adf_schema_1.isSchemaWithEmojis(this.schema) && node.type === emoji) {
            return this.encodeEmoji(node);
        }
        if (adf_schema_1.isSchemaWithCodeBlock(this.schema) && node.type === codeBlock) {
            return this.encodeCodeBlock(node);
        }
        if (adf_schema_1.isSchemaWithBlockQuotes(this.schema) && node.type === blockquote) {
            return this.encodeBlockQuote(node);
        }
        if (adf_schema_1.isSchemaWithMedia(this.schema)) {
        ...
        if (adf_schema_1.isSchemaWithTables(this.schema) && node.type === table) {
...
exports.JIRATransformer = JIRATransformer;
