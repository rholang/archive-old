...
function ensureBlocks(fragment, schema, nodeType) {
    ...
    return prosemirror_model_1.Fragment.fromArray(blockNodes);
}
exports.ensureBlocks = ensureBlocks;
/**
 * This function will convert all content to inline nodes
 */
exports.ensureInline = function (schema, content, supportedMarks) {
    var result = [];
...
function convert(content, node, schema) {
    // text
    ...
    if (node instanceof HTMLElement) {
        ...
        switch (tag) {
            // Marks
            case 'DEL':
                if (!adf_schema_1.isSchemaWithAdvancedTextFormattingMarks(schema)) {
                    return null;
                }
            ...
            case 'SUB':
            case 'SUP':
                if (!adf_schema_1.isSchemaWithSubSupMark(schema)) {
                    return null;
                }
            ...
            case 'FONT':
                if (!adf_schema_1.isSchemaWithTextColor(schema)) {
                    return null;
                }
                var color = color_1.normalizeHexColor(node.getAttribute('color'), '#333333');
                return color
                    ? addMarks(content, [schema.marks.textColor.create({ color: color })])
                    : content;
            // Nodes
            case 'A':
                if (node.className === 'user-hover' && adf_schema_1.isSchemaWithMentions(schema)) {
                ...
                if (node.className.match('jira-issue-macro-key') ||
                    !content ||
                    !adf_schema_1.isSchemaWithLinks(schema)) {
                    return null;
                }
            ...
            case 'SPAN':
                ...
                if (node.className.split(' ').indexOf('jira-issue-macro') > -1) {
                    ...
                    if (jiraKey && link) {
                        return addMarks(prosemirror_model_1.Fragment.from(schema.text(jiraKey)), [
                ...
                else if (isMedia(node) && adf_schema_1.isSchemaWithMedia(schema)) {
            ...
            case 'IMG':
                if (node.parentElement &&
                    ...
                    var emojiResult = emojiHelper_1.mapImageToEmoji(node);
                    if (emojiResult) {
            ...
            case 'H6':
                ...
                { level: level === 6 ? 5 : level }, schema.nodes.heading.validContent(content)
                    ? content
                    : exports.ensureInline(schema, content, supportedMarks));
            case 'BR':
            ...
            case 'P':
                if (node.firstElementChild && isMedia(node.firstElementChild)) {
                    ...
                    var _a = schema.nodes, mediaSingle = _a.mediaSingle, mediaGroup = _a.mediaGroup, paragraph = _a.paragraph;
                    var mediaArray_1 = [];
                    var contentArray_1 = [];
                    var fragmentArray = [];
                    ...
                    content.forEach(function (child) {
                        if (child.type === schema.nodes.media) {
                            mediaArray_1.push(child);
                            return;
                        }
                        else if (!(child.isText && /^\s*$/.test(child.text || ''))) {
                            hasNonMediaChildren_1 = true;
                        }
                        contentArray_1.push(child);
                    });
                    if (hasNonMediaChildren_1 && contentArray_1.length) {
                        fragmentArray.push(paragraph.createChecked({}, contentArray_1));
                    }
                    if (adf_schema_1.isSchemaWithMedia(schema) && mediaArray_1.length) {
                        var mediaNodeType = isMediaSingle(node.firstElementChild)
                            ? mediaSingle
                            : mediaGroup;
                        fragmentArray.push(mediaNodeType.createChecked({}, mediaArray_1));
                    }
                    if (fragmentArray.length) {
                        return prosemirror_model_1.Fragment.fromArray(fragmentArray);
                    }
                    return null;
                }
        ...
        // lists
        if (adf_schema_1.isSchemaWithLists(schema)) {
            switch (tag) {
                case 'UL':
        ...
        // code block
        if (adf_schema_1.isSchemaWithCodeBlock(schema)) {
            switch (tag) {
                case 'DIV':
        ...
        if (adf_schema_1.isSchemaWithBlockQuotes(schema) && tag === 'BLOCKQUOTE') {
        ...
        // table
        if (adf_schema_1.isSchemaWithTables(schema)) {
            switch (tag) {
                case 'TABLE':
