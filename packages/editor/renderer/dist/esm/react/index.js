...
var ReactSerializer = /** @class */ (function () {
    ...
    ReactSerializer.prototype.serializeFragment = function (fragment, props, target, key, parentInfo) {
        ...
        var content = ReactSerializer.getChildNodes(fragment).map(function (node, index) {
            ...
            var pInfo = {
                ...
                path: __spread(currentPath, [node]),
            };
    ...
    ReactSerializer.prototype.renderNode = function (NodeComponent, props, key, content) {
        return (React.createElement(NodeComponent, __assign({ key: key }, props), content));
    };
    ReactSerializer.prototype.renderMark = function (MarkComponent, props, key, content) {
        return (React.createElement(MarkComponent, __assign({ key: key }, props), content));
    };
    ReactSerializer.prototype.getTableChildrenProps = function (node) {
        return __assign(__assign({}, this.getProps(node)), { allowColumnSorting: this.allowColumnSorting });
    };
    ReactSerializer.prototype.getTableProps = function (node) {
        return __assign(__assign({}, this.getProps(node)), { allowColumnSorting: this.allowColumnSorting, columnWidths: calcTableColumnWidths(node), tableNode: node });
    };
    ...
    ReactSerializer.prototype.getHeadingProps = function (node, path) {
        if (path === void 0) { path = []; }
        return __assign(__assign({}, node.attrs), { content: node.content ? node.content.toJSON() : undefined, headingId: this.getHeadingId(node), showAnchorLink: this.allowHeadingAnchorLinks &&
    ...
    ReactSerializer.prototype.getMarkProps = function (mark) {
        var _a = mark.attrs, key = _a.key, otherAttrs = __rest(_a, ["key"]);
        return __assign({ eventHandlers: this.eventHandlers, markKey: key }, otherAttrs);
    };
    ...
    ReactSerializer.buildMarkStructure = function (content) {
        ...
        return mergeMarks(content.map(function (node) {
            ...
            return nodeMarks.reverse().reduce(function (acc, mark) {
                var eq = mark.eq;
                return __assign(__assign({}, mark), { eq: eq, content: [acc] });
            }, node);
        }));
    };
