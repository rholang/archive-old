...
exports.getNestedDepth = function (conversation, parentId, level) {
    ...
    return exports.getNestedDepth(conversation, parent.parentId, level + 1);
};
var updateComment = function (comments, newComment) {
    return (comments || []).map(function (comment) {
        if ((newComment.localId && comment.localId === newComment.localId) ||
            ...
            return tslib_1.__assign(tslib_1.__assign(tslib_1.__assign({}, comment), { oldDocument: comment.oldDocument || comment.document }), newComment);
        }
        return comment;
    });
};
...
var updateCommentInConversation = function (conversations, newComment) {
    return conversations.map(function (conversation) {
        if (conversation.conversationId === newComment.conversationId) {
            ...
            return tslib_1.__assign(tslib_1.__assign({}, conversation), { comments: comments });
        }
        return conversation;
    });
};
    ...
    return conversations.map(function (conversation) {
        if (conversation.conversationId === newComment.conversationId) {
            ...
            if (newComment.localId &&
                ...
                return tslib_1.__assign(tslib_1.__assign({}, conversation), { comments: updateComment(comments, newComment) });
            }
            newComment.nestedDepth = exports.getNestedDepth(conversation, newComment.parentId);
            // Otherwise, add it
            return tslib_1.__assign(tslib_1.__assign({}, conversation), { comments: tslib_1.__spread(comments, [newComment]) });
        }
        return conversation;
    });
};
    ...
    return conversations.reduce(function (current, conversation) {
        if (conversation.conversationId === commentToRemove.conversationId) {
            ...
            return tslib_1.__spread(current, [
                tslib_1.__assign(tslib_1.__assign({}, conversation), { comments: comments }),
            ]);
        }
...
exports.reducers = create_reducer_1.createReducer(exports.initialState, (_a = {},
    _a[actions_1.FETCH_CONVERSATIONS_REQUEST] = function (state) {
    ...
    _a[actions_1.FETCH_CONVERSATIONS_SUCCESS] = function (state, action) {
        var leveledConversations = action.payload.map(function (conversation) {
            ...
            conversation.comments = conversation.comments.map(function (comment) { return (tslib_1.__assign(tslib_1.__assign({}, comment), { nestedDepth: exports.getNestedDepth(conversation, comment.parentId) })); });
            return conversation;
        });
        var conversations = tslib_1.__spread(leveledConversations);
        return tslib_1.__assign(tslib_1.__assign({}, state), { conversations: conversations });
    },
    _a[actions_1.ADD_COMMENT_REQUEST] = function (state, action) {
        var payload = action.payload;
        var conversations = addOrUpdateCommentInConversation(state.conversations, tslib_1.__assign(tslib_1.__assign({}, payload), { isPlaceholder: true, state: 'SAVING' }));
        return tslib_1.__assign(tslib_1.__assign({}, state), { conversations: conversations });
    },
    _a[actions_1.ADD_COMMENT_SUCCESS] = function (state, action) {
        var payload = action.payload;
        var conversations;
        conversations = addOrUpdateCommentInConversation(state.conversations, tslib_1.__assign(tslib_1.__assign({}, payload), { state: undefined, oldDocument: undefined, isPlaceholder: false }));
        return tslib_1.__assign(tslib_1.__assign({}, state), { conversations: conversations });
    },
    _a[actions_1.ADD_COMMENT_ERROR] = function (state, action) {
        var payload = action.payload;
        var conversations = updateCommentInConversation(state.conversations, tslib_1.__assign(tslib_1.__assign({}, payload), { state: 'ERROR' }));
        return tslib_1.__assign(tslib_1.__assign({}, state), { conversations: conversations });
    },
    _a[actions_1.UPDATE_COMMENT_REQUEST] = function (state, action) {
        var payload = action.payload;
        var conversations = updateCommentInConversation(state.conversations, tslib_1.__assign(tslib_1.__assign({}, payload), { state: 'SAVING' }));
        return tslib_1.__assign(tslib_1.__assign({}, state), { conversations: conversations });
    },
    _a[actions_1.UPDATE_COMMENT_SUCCESS] = function (state, action) {
        var payload = action.payload;
        var conversations = updateCommentInConversation(state.conversations, tslib_1.__assign(tslib_1.__assign({}, payload), { state: undefined, oldDocument: undefined }));
        return tslib_1.__assign(tslib_1.__assign({}, state), { conversations: conversations });
    },
    ...
    _a[actions_1.DELETE_COMMENT_SUCCESS] = function (state, action) {
        var payload = action.payload;
        var conversations = updateCommentInConversation(state.conversations, tslib_1.__assign(tslib_1.__assign({}, payload), { state: undefined, deleted: true, oldDocument: undefined }));
        return tslib_1.__assign(tslib_1.__assign({}, state), { conversations: conversations });
    },
    ...
    _a[actions_1.REVERT_COMMENT] = function (state, action) {
        ...
        if (comment.isPlaceholder) {
            ...
            conversations = updateCommentInConversation(state.conversations, tslib_1.__assign(tslib_1.__assign({}, payload), { state: undefined, document: comment.oldDocument, deleted: false, oldDocument: undefined }));
        }
        return tslib_1.__assign(tslib_1.__assign({}, state), { conversations: conversations });
    },
    _a[actions_1.HIGHLIGHT_COMMENT] = function (state, action) {
        ...
        return tslib_1.__assign(tslib_1.__assign({}, state), { highlighted: highlighted });
    },
    _a[actions_1.UPDATE_USER_SUCCESS] = function (state, action) {
        return tslib_1.__assign(tslib_1.__assign({}, state), { user: action.payload.user });
    },
    _a[actions_1.CREATE_CONVERSATION_REQUEST] = function (state, action) {
        ...
        var conversations = tslib_1.__spread(state.conversations, [
            tslib_1.__assign(tslib_1.__assign({}, payload), { comments: [
                    tslib_1.__assign(tslib_1.__assign({}, comment), { state: 'SAVING', isPlaceholder: true }),
                ] }),
        ]);
        return tslib_1.__assign(tslib_1.__assign({}, state), { conversations: conversations });
    },
    ...
    _a[actions_1.CREATE_CONVERSATION_ERROR] = function (state, action) {
        ...
        var conversations = updateCommentInConversation(state.conversations, tslib_1.__assign(tslib_1.__assign({}, comment), { state: 'ERROR', error: error }));
        return tslib_1.__assign(tslib_1.__assign({}, state), { conversations: conversations });
    },
    _a));
